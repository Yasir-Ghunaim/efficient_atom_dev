...
@torch.no_grad()
def mmd_linear_rbf(X, Y, s2=None):
    # even counts for pairing
    n = (X.size(0)//2)*2
    m = (Y.size(0)//2)*2
    permX = torch.randperm(X.size(0), device=X.device)[:n]
    permY = torch.randperm(Y.size(0), device=Y.device)[:m]
    X = X[permX]; Y = Y[permY]
    X1, X2 = X[0::2], X[1::2]
    Y1, Y2 = Y[0::2], Y[1::2]

    # cross pairs must be same length
    k = min(X1.size(0), Y1.size(0))
    X1, X2 = X1[:k], X2[:k]
    Y1, Y2 = Y1[:k], Y2[:k]

    # bandwidth via small subset (already cheap)
    if s2 is None:
        s2 = _median_bandwidth_sq(X1, Y1)

    def k_pair(a, b):
        d2 = (a - b).pow(2).sum(dim=1)           # shape: (k,)
        return torch.exp(-d2 / (2 * s2))         # elementwise

    term_x  = k_pair(X1, X2).mean()
    term_y  = k_pair(Y1, Y2).mean()
    term_xy = k_pair(X1, Y2).mean()
    term_yx = k_pair(Y1, X2).mean()
    return (term_x + term_y - term_xy - term_yx).item()
...
def main():
    ...
    mmd_score = mmd_linear_rbf(upstream_features, downstream_features)
