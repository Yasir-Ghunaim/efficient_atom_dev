...
class AtomicData:
    def __init__(
        ...
        self.fixed = fixed
        self.tags = tags
        # self.sid = sid if isinstance(sid, torch.Tensor) else torch.tensor(sid, dtype=torch.long)

        if isinstance(sid, torch.Tensor):
            self.sid = sid
        elif isinstance(sid, int):
            self.sid = torch.tensor([sid])
        elif isinstance(sid, str) and sid.isdigit():
            self.sid = torch.tensor([int(sid)])
        else:
            self.sid = torch.tensor([-1])  # fallback
    ...
    def validate(self):
        ...
        # print("=========", self.fixed.dtype)
        assert self.tags.dtype == torch.long
        ...
        if self.sid is not None:
            ...
            assert isinstance(self.sid, torch.Tensor)
            assert len(self.sid) == self.num_graphs
    ...
    def __inc__(self, key, value) -> int:
        ...
        assert self.num_graphs == 1
        return self.natoms if bool(re.search("index", key)) else 0
...
def atomicdata_list_to_batch(
    ...
    cat_dims = {}
    natoms_list, sid_list, fid_list, lmdb_idx_list = [], [], [], []
    ...
    for i, data in enumerate(data_list):
        ...
        # sid_list.extend([data.sid])
        sid_list.append(torch.tensor([data.sid]) if isinstance(data.sid, int) else data.sid)
        fid_list.append(torch.tensor([data.fid]) if isinstance(data.fid, int) else data.fid)
        lmdb_idx_list.append(torch.tensor([data.lmdb_idx]) if isinstance(data.lmdb_idx, int) else data.lmdb_idx)
