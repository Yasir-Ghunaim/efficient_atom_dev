...
class LmdbDataset(Dataset[T], ContextDecorator):
    ...
    def __init__(
        ...
        if hasattr(args, "extract_features"):
            self.extract_features = args.extract_features
        else:
        ...
        elif self.max_samples > self.num_samples:
            self.max_samples = self.num_samples
        ...
        if self.extract_features:
            self.shuffled_indices = self._apply_sampling(args)
        else:
