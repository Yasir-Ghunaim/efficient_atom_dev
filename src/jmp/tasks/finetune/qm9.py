...
class SpatialExtentOutputHead(nn.Module):
    @override
    def __init__(self, config, atomic_masses: Callable[[], torch.Tensor], reduction: str):
        super().__init__()

        if reduction is None:
            reduction = config.graph_scalar_reduction_default
        assert reduction == "sum", f"reduction must be sum, got {self.reduction=}"

        dim = config.backbone.emb_size_atom
        scalar_mlp_layers: list[nn.Module] = []
        for _ in range(config.output.num_mlps):
            scalar_mlp_layers.append(nn.Linear(dim, dim, bias=False))
            scalar_mlp_layers.append(config.activation_cls())
