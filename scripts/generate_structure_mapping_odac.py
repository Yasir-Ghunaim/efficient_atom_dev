    ...
    with env.begin() as txn:
        ...
        for idx in tqdm(range(length), desc=f"Processing {lmdb_file.name}", unit="record", position=position, leave=False):
            ...
            try:
                ...
                molecule_symbol = record.get("name", "Unknown").split("_")[0]
...
def process_all_lmdbs(lmdb_dir, output_dir, num_workers=10):
    lmdb_files = sorted(
        ...
        key=lambda x: int(x.stem.split('.')[0])  # Sort by numeric part of filename
    )
...
if __name__ == "__main__":
    ...
    # TODO: Update this path
    lmdb_dir = "/ibex/project/c2261/datasets/odac/s2ef/train/"
