...
def process_all_lmdbs(lmdb_dir, output_dir, num_workers=10):
    ...
    output_files = [Path(output_dir) / ("data_log." + f.name.replace(".lmdb", ".txt")) for f in lmdb_files]
    ...
    for i, (lmdb_file, output_file) in enumerate(zip(lmdb_files, output_files)):
        print(f"Processing {lmdb_file} -> {output_file}")
        process_lmdb((lmdb_file, output_file, i))
...
if __name__ == "__main__":
    ...
    # TODO: Update this path
    lmdb_dir = "/ibex/project/c2261/datasets/qm9/lmdb/train/"
    ...
    # Run the processing
    output_dir = lmdb_dir
    process_all_lmdbs(lmdb_dir, output_dir, num_workers=100)